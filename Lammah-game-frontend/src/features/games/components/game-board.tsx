...
export function GameBoard({ gameId }: GameBoardProps) {
  ...
  const [selectedBoardQuestion, setSelectedBoardQuestion] =
    useState<BoardQuestion | null>(null);
  ...
  const selectedQuestion =
    selectedBoardQuestion?.question || game.currentQuestion;
  ...
  const findRevealedBoardQuestion = (questionId: string, revealedGame = game) =>
    revealedGame.board
      ?.filter(Array.isArray)
      .flat()
      .find((boardQuestion) => {
        const populatedQuestionId = boardQuestion.question
          ? getEntityId(boardQuestion.question)
          : "";
        return (
          boardQuestion.questionId === questionId ||
          populatedQuestionId === questionId ||
          getEntityId(boardQuestion) === questionId
        );
      });
  ...
  const handleRevealAnswer = () => {
    ...
    revealAnswer.mutate(selectedQuestionId, {
      onSuccess: (revealedGame) => {
        const revealedBoardQuestion = findRevealedBoardQuestion(
          selectedQuestionId,
          revealedGame,
        );

        if (revealedBoardQuestion) {
          setSelectedBoardQuestion(revealedBoardQuestion);
        }
