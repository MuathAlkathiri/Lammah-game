...
import { toMusicUploadRequest, useUploadMusicTrack } from "@/features/music";
  ...
  const onSubmit = async (data: QuestionFormData) => {
    ...
    try {
      if (data.type === "audio") {
        ...
        const uploadRequest = toMusicUploadRequest(audioFile, {
          title: data.musicTitle,
          artist: data.musicArtist,
          album: data.musicAlbum,
          language: data.musicLanguage,
          genre: data.musicGenre,
          difficulty: data.difficulty,
          snippetDurationSeconds: data.snippetDurationSeconds,
          snippetStartSecond: data.snippetStartSecond,
        });

        await uploadMusicTrack.mutateAsync(uploadRequest);
        showToast({
          type: "success",
