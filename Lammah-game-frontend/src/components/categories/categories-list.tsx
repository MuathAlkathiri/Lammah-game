...
export function CategoriesList() {
  ...
  if (!categories.length) return <div className="text-center py-8">لا توجد فئات</div>;

  return (
    <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
      {categories.map((category) => (
        <Card key={getEntityId(category)} className="overflow-hidden">
          <CardHeader>
            <div className="flex items-start justify-between gap-3">
              <div>
                <CardTitle className="text-2xl font-black">{category.name}</CardTitle>
                <CardDescription>{category.description}</CardDescription>
                <p className="text-xs text-muted-foreground mt-2">{category.slug}</p>
              </div>
              <div className="flex flex-col items-end gap-2">
                ...
                <Button
                  size="sm"
                  variant="destructive"
                  onClick={() => deleteCategory.mutate(getEntityId(category))}
