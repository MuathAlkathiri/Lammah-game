...
import { useCatalogs, useCategories, useDeleteCatalog } from '@/lib/hooks';
...
export function CatalogsList() {
  ...
  if (error) return <div className="py-8 text-center text-destructive">حدث خطأ</div>;
  if (!catalogs?.length) return <div className="py-8 text-center">لا توجد كتالوجات</div>;
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <Badge variant="secondary">{linkedCategoryCount} فئة</Badge>
                    </div>
                  </div>
                  <div className="flex flex-col gap-2">
                    <Button size="sm" variant="outline" onClick={() => setEditingCatalog(catalog)}>
                      تعديل
                    </Button>
                    <Button
                      size="sm"
